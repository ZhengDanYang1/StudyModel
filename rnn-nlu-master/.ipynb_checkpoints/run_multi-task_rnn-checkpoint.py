...
def get_perf(filename):
    ...
    strin = open(filename).readlines()
    strin = ''.join(strin)
    print('open(filename).readlines()', type(strin))
    stdout, _ = proc.communicate(strin.encode('utf-8'))
    print('stdout',stdout)
...
def create_model(session, 
  ...
  with tf.variable_scope("model", reuse=None):
    model_train = multi_task_model.MultiTaskModel(
          ...
          label_vocab_size, 
          _buckets,
          FLAGS.word_embedding_size, #128
          FLAGS.size, FLAGS.num_layers, #128, 1
          FLAGS.max_gradient_norm, #5
          FLAGS.batch_size,
          dropout_keep_prob=FLAGS.dropout_keep_prob, #0.5
          use_lstm=True,
          forward_only=False, 
...
def train():
  ...
  date_set = data_utils.prepare_multi_task_data(
    FLAGS.data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size) # data/ATIS_samples， 10000, 10000
  #print('date_set',date_set)
  #[('data/ATIS_samples/train/train.ids10000.seq.in', 'data/ATIS_samples/train/train.ids10000.seq.out', 'data/ATIS_samples/train/train.ids.label'), ('data/ATIS_samples/valid/valid.ids10000.seq.in', 'data/ATIS_samples/valid/valid.ids10000.seq.out', 'data/ATIS_samples/valid/valid.ids.label'), ('data/ATIS_samples/test/test.ids10000.seq.in', 'data/ATIS_samples/test/test.ids10000.seq.out', 'data/ATIS_samples/test/test.ids.label'), ('data/ATIS_samples/in_vocab_10000.txt', 'data/ATIS_samples/out_vocab_10000.txt', 'data/ATIS_samples/label.txt')]
  ...
  config = tf.ConfigProto(
      gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.23),#设置GPU占用量
      #device_count = {'gpu': 2}
  )
    
  with tf.Session(config=config) as sess:
    # Create model.
    print("Max sequence length: " ,_buckets)
    ...
    #data_set [[[[10, 4, 11, 7, 8, 12, 3, 13, 2, 9], [2, 2, 7, 5, 6, 2, 2, 4, 2, 3], [1]], [[14, 15, 4, 16, 17, 3, 5, 2, 9, 6, 18, 19, 20, 4, 21], [2, 2, 2, 8, 2, 2, 4, 2, 3, 2, 2, 2, 2, 2, 2], [2]], [[22, 23, 24, 3, 5, 2, 25, 6, 26, 27, 28], [2, 2, 2, 2, 4, 2, 3, 2, 9, 10, 11], [2]], [[29, 30, 31, 32, 2, 33, 7, 8], [12, 2, 4, 13, 2, 3, 5, 6], [1]], [[34, 35, 2, 36, 3, 5, 2, 37, 6, 2, 38, 39, 40, 41], [2, 2, 2, 2, 2, 4, 2, 3, 2, 2, 3, 14, 2, 2], [2]]]]
