...
class MultiTaskModel(object):
  def __init__(self, 
    ...
    self.word_embedding_size = word_embedding_size #128
    self.cell_size = size #128
    ...
    for i in xrange(buckets[-1][0]):
      self.encoder_inputs.append(tf.placeholder(tf.int32, shape=[None],#一个单词一个placeholder
                                                name="encoder{0}".format(i)))
    #print('self.encoder_inputs', self.encoder_inputs)
    
    print('buckets', buckets)
    for i in xrange(buckets[-1][1]):
      self.tags.append(tf.placeholder(tf.float32, shape=[None], #一个tags一个placeholder
                                      name="tag{0}".format(i)))
      self.tag_weights.append(tf.placeholder(tf.float32, shape=[None],
                                                name="weight{0}".format(i)))
    #print('self.tags',self.tags)
    #print('self.tag_weights',self.tag_weights)
  ...
  def generate_rnn_output(self):
    ...
    with tf.variable_scope("generate_seq_output"):
      if self.bidirectional_rnn:
        ...
        encoder_state = tf.concat([tf.concat(state_fw, 1),#编码器最后状态 shape=(?, 512)
                                   tf.concat(state_bw, 1)], 1)
        #print('encoder_state',encoder_state)
        top_states = [tf.reshape(e, [-1, 1, self.cell_fw.output_size \
                                  + self.cell_bw.output_size])
                      for e in encoder_outputs]#encoder_outputs是一个长度为T的list，list中的每个元素对应每个时间步的输出，它们是深度级联的前向和反向输出
        attention_states = tf.concat(top_states, 1)#shape=(?, 50, 256)
